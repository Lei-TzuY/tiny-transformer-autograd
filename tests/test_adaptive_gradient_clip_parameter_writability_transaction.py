...
class MakeParameterReadOnlyOnWrite(np.ndarray):
...
def test_commit_detects_and_restores_parameter_data_writability():
    ...
    gradient = MakeParameterReadOnlyOnWrite([6.0, 8.0])
    ...
    with pytest.raises(RuntimeError, match="parameter data writability changed for parameter 0"):
        adaptive_clip_grad_(parameter, clip_factor=0.1)

    assert np.asarray(parameter.data).flags.writeable
