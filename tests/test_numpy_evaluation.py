...
def test_numpy_cross_entropy_matches_current_nonfinite_scored_contract(bad):
    logits = np.array([[[1.0, bad], [0.5, -0.5]]], dtype=np.float64)
    targets = np.array([[0, 1]], dtype=np.int64)

    with pytest.raises(ValueError, match=r"NaN or \+inf"):
        ops.cross_entropy(Tensor(logits), targets)
    with pytest.raises(ValueError, match=r"NaN or \+inf"):
        _cross_entropy_np(logits, targets)


def test_numpy_cross_entropy_ignores_nonfinite_unscored_rows():
    logits = np.array([[[2.0, 0.0], [np.nan, np.inf]]], dtype=np.float64)
    targets = np.array([[0, IGNORE_INDEX]], dtype=np.int64)
