...
def test_recompute_skips_unused_requires_grad_input():
    ...
    out = recompute(lambda used, ignored: ops.sum(used * used), x, unused)
    out.backward()
...
def test_recompute_preserves_existing_grad_on_unused_input():
    ...
    out = recompute(lambda used, ignored: ops.sum(used * 2.0), x, unused)
    out.backward()
...
def test_recompute_unused_input_does_not_block_parameter_gradient():
    ...
    out = recompute(lambda used, ignored: ops.sum(used * weight), x, unused)
    out.backward()
...
def test_recompute_duplicate_input_only_accumulates_used_replay_copy():
    ...
    out = recompute(lambda used, ignored: ops.sum(used * 4.0), x, x)
    out.backward()
