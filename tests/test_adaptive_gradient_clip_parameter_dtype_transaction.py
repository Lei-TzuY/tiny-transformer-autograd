...
class ChangeParameterDtypeOnWrite(np.ndarray):
...
def test_commit_detects_and_restores_parameter_data_dtype_change():
    ...
    gradient = ChangeParameterDtypeOnWrite([1.0, -1.0])
    ...
    with pytest.raises(RuntimeError, match="parameter data dtype changed for parameter 0"):
        adaptive_clip_grad_(parameter, clip_factor=0.1)

    assert parameter.data.dtype == np.dtype(np.float64)
